...
class ForkJoinSpec extends AbstractSpecification {
    ...
    @Shared
    def FORK_JOIN_EXCLUSIVE_WF = 'FanInOutExclusiveTest'
    ...
    def setup() {
        workflowTestUtil.registerWorkflows('fork_join_integration_test.json',
                ...
                'fork_join_exclusive_integration_test.json',
        )
    }
     ...
     *             End
     */
    def "Test a simple workflow with fork join exclusive failure flow"() {
        ...
        when: "A fork join workflow is started"
        def workflowInstanceId = startWorkflow(FORK_JOIN_EXCLUSIVE_WF, 1,
                'fanoutTest', [:],
                null)
        ...
        with(workflowExecutionService.getExecutionStatus(workflowInstanceId, true)) {
            ...
            tasks[0].status == Task.Status.COMPLETED
            tasks[0].taskType == 'FORK'
            tasks[1].workflowTask.type == TaskType.EXCLUSIVE.name()
            tasks[1].status == Task.Status.SCHEDULED
            tasks[1].taskType == 'integration_task_1'
            tasks[2].workflowTask.type == TaskType.EXCLUSIVE.name()
            tasks[2].status == Task.Status.SCHEDULED
            tasks[2].taskType == 'integration_task_2'
            tasks[3].status == Task.Status.IN_PROGRESS
            tasks[3].taskType == 'JOIN'
        ...
        with(workflowExecutionService.getExecutionStatus(workflowInstanceId, true)) {
            ...
            tasks[3].taskType == 'JOIN'
            tasks[4].status == Task.Status.CANCELED // todo: toggle cancelNonTerminalTasks exclusive tasks handling for SCHEDULED
            tasks[4].taskType == 'integration_task_3'
        }
